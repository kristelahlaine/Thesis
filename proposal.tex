...
{\bf\Large Solving Sparse Sudoku Puzzles with Dancing Links Through Reduction }\\
...
In 2014, Harrysson described and implemented the reduction of human-playable Sudokus with clues ranging from 17 to 31 and observed that the while it is faster to reduce Sudoku puzzles with more clues, the difficulty of finding the solution does not reside in the number of clues, but on something else  \cite{Harrysson}. The motivation of this paper is to describe and implement the reduction of puzzles that were not covered by \cite{Harrysson} : Sparse Sudokus. We define Sparse Sudoku puzzles as Sudoku puzzles with clues ranging from 0 to 16 thus making the Sudoku puzzle grid less than 20\% filled. The process will involve reduction of sudoku to DLX. Runtime of the reduction and of solving the solutions will be observed and will be compared with the general observations of Harryson. Experiments will be conducted to selected Sparse Suduko puzzles.\\
%Abstract
...
Exploring Sudoku puzzles ranging from 17 clues to 31 clues, the reduction of \cite{Harrysson} shows promising results in solving the Sudoku in its Exact Cover equivalent. However, it remains to show how the reduction process can be scaled for Sparse Sudoku puzzles and whether it also affects the runtime.\\

\section{Objectives of the Study}
The aim of this study is to reduce Sparse Sudoku puzzles to its Exact Cover equivalent and implementing the DLX algorithm. Specifically:
\begin{enumerate}
...
\section{Limitations}
This study focused on continuing the study of Harrysson but with the focus on exploring Sparse Sudoku puzzles; which were not explored in the study of Harrysson. At the same time, this study is limited on solving 9$\times$9 Sudoku problems and not its other variants. This study only emphasizes on the reduction and on solving the Exact Cover equivalent using DLX.

\section{Significance}
According to \cite{Harrysson}, the reduction process is arguably the hardest part in the study since there is not much detailed literature about the reduction process. By solving Sparse Sudoku puzzles in its Exact Cover equivalent, this study supplements previous studies that tackle solving Sudoku through reduction. Also, this study provides a description on how the reduction process  of Sparse Sudoku puzzles to its Exact Cover equivalent is done. This will also hope to spark the interest of other software engineers to take advantage of reduction as a mechanism to solve real-world problems.
...
\noindent In this study, the reduction of Sparse Sudoku puzzles to Exact Cover will have two implications:
\begin{itemize}
\item That the difficulty of the Sparse Sudoku is still not based on the number of clues.
...
The matrix will then be evaluated with the use of the Dancing Links technique. The results of this implementation will be compared to the results of the Brute-force implementation and also to the results in \cite{Harrysson}.
