...
When solving the Sudoku through its Exact Cover equivalent, the difficulty does not reside in the number of clues, but on something else \cite{Harrysson}. While the reduction of \cite{Harrysson} shows promising results in solving the Sudoku in its Exact Cover equivalent, it remains to show how the reduction process can be scaled for Sudoku puzzles with less than 17 clues and whether if it also affects the runtime.\\
...
This study intends to translate into an algorithm the description of \cite{Harrysson} in reducing Sudoku problems with less than 17 clues to its Exact Cover equivalent and find solutions of each problem using the Dancing Links.

\section{Objectives of the Study}
The aim of this study is to solve non-human-playable Sudoku problems using the Dancing Links algorithm using the reduction process of the Sudoku problems to instances of Exact Cover as described by \cite{Harrysson}. Specifically:
\begin{enumerate}
