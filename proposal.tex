...
When solving the Sudoku through its Exact Cover equivalent, the difficulty does not reside in the number of clues, but on something else \cite{Harrysson}. Exploring human-playable Sudoku puzzles (17 or more clues), the reduction of \cite{Harrysson} shows promising results in solving the Sudoku in its Exact Cover equivalent. However, it remains to show how the reduction process can be scaled for multi-solution Sudoku puzzles and whether if it also affects the runtime. **Add Results of Mattias Harrysson**\\

\begin{figure}[h]
  \centering
  \subfloat[Solving 0 - 16 clue Sudoku]{\includegraphics[width=0.9\textwidth]{framework.jpg}\label{fig:framework}}
  \caption{}
\end{figure}
